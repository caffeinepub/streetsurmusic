...
interface LocalProfileContextValue {
  ...
  addPlaylist: (pl: Omit<LocalPlaylist, "id">) => void;
  editPlaylist: (
    id: string,
    updates: Partial<Omit<LocalPlaylist, "id">>,
  ) => void;
...
const LocalProfileContext = createContext<LocalProfileContextValue | null>(
  null,
);
...
export function LocalProfileProvider({
  ...
  const updatePhoto = useCallback(
    (photo: string | null) => update({ photo }),
    [update],
  );
  const updateBio = useCallback((bio: string) => update({ bio }), [update]);
...
export function useLocalProfileContext(): LocalProfileContextValue {
  const ctx = useContext(LocalProfileContext);
  if (!ctx)
    throw new Error(
      "useLocalProfileContext must be used inside LocalProfileProvider",
    );
  return ctx;
}
